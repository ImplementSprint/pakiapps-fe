...
export function CustomerSettingsPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // Keep local fallback values if backend preferences are unavailable.
      }
    })();
  ...
  const togglePreference = async (key: keyof typeof preferences) => {
    ...
    try {
      await updateCustomerPreferences({ [key]: newPreferences[key] });
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Booking, tracking, payments, and account answers</p>
                </div>
              </div>
              <ArrowLeft className="w-4 h-4 text-gray-400 rotate-180" />
            </button>
